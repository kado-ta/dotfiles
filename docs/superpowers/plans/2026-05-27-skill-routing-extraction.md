# Skill Routing 切り出し 実装計画

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** `CLAUDE.md` の Skill Routing セクションと `docs/SKILL_ROUTING.md` を `.claude/skills/skill-routing/SKILL.md` に統合し、`using-superpowers` フロー経由で自動起動されるスキルとして切り出す。

**Architecture:** 単一の `SKILL.md` ファイルに全ルーティングロジックを集約。`description` フィールドに「すべてのタスク開始時に使う」旨を記載し、`using-superpowers` の 1% ルールで確実に起動されるようにする。`CLAUDE.md` は 1 行ポインターのみ残し、`docs/SKILL_ROUTING.md` は削除する。

**Tech Stack:** Markdown、Bash（setup.sh）

---

### Task 1: skill-routing スキルファイルを作成する

**Files:**
- Create: `.claude/skills/skill-routing/SKILL.md`

- [ ] **Step 1: ディレクトリを作成する**

```bash
mkdir -p /Users/kado/dotfiles/.claude/skills/skill-routing
```
...
`.claude/skills/skill-routing/SKILL.md` を以下の内容で作成する:
...
```bash
ls -la /Users/kado/dotfiles/.claude/skills/skill-routing/
head -5 /Users/kado/dotfiles/.claude/skills/skill-routing/SKILL.md
```

Expected: `SKILL.md` が存在し、先頭に `---` (frontmatter) が見える。

- [ ] **Step 4: コミットする**

```bash
cd /Users/kado/dotfiles
git add .claude/skills/skill-routing/SKILL.md
git commit -m "feat(skills): add skill-routing skill
...
### Task 2: CLAUDE.md の Skill Routing セクションを 1 行に縮小する

**Files:**
- Modify: `.claude/CLAUDE.md`（127〜153 行の Skill Routing セクション）

- [ ] **Step 1: 変更前の行数を確認する**

```bash
grep -n "## Skill Routing" /Users/kado/dotfiles/.claude/CLAUDE.md
```

Expected: `127:## Skill Routing` のような出力。

- [ ] **Step 2: Skill Routing セクションを 1 行ポインターに書き換える**

`## Skill Routing` から始まるセクション末尾（ファイル末尾）を以下に置き換える:
...
（`## Skill Routing` から始まる現在の30行を、上記2行に置き換える。他のセクションは触れない。）

- [ ] **Step 3: 変更内容を確認する**

```bash
grep -A 3 "## Skill Routing" /Users/kado/dotfiles/.claude/CLAUDE.md
```
...
git commit -m "refactor(claude-md): replace Skill Routing section with 1-line pointer to skill
...
### Task 3: docs/SKILL_ROUTING.md を削除する

**Files:**
- Delete: `.claude/docs/SKILL_ROUTING.md`

- [ ] **Step 1: ファイルを削除する**

```bash
git rm /Users/kado/dotfiles/.claude/docs/SKILL_ROUTING.md
```

- [ ] **Step 2: docs/ に他のファイルが残っているか確認する**

```bash
ls /Users/kado/dotfiles/.claude/docs/
```

Expected: `CODEX_OFFLOAD.md` のみ残る（`SKILL_ROUTING.md` が消えている）。

- [ ] **Step 3: コミットする**

```bash
cd /Users/kado/dotfiles
git commit -m "chore(docs): remove SKILL_ROUTING.md (absorbed into skill-routing skill)
...
### Task 4: シンボリックリンクを作成して動作確認する

**Files:**
- (symlink) `~/.claude/skills/skill-routing` → `.claude/skills/skill-routing`

- [ ] **Step 1: setup.sh を実行してシンボリックリンクを作成する**

```bash
bash /Users/kado/dotfiles/.claude/setup.sh
```

Expected: `~/.claude/skills/skill-routing -> ...` のような出力が含まれる。

- [ ] **Step 2: シンボリックリンクが正しく作成されたか確認する**

```bash
ls -la ~/.claude/skills/ | grep skill-routing
```

Expected:
```
skill-routing -> /Users/kado/dotfiles/.claude/skills/skill-routing
```

- [ ] **Step 3: スキルファイルがグローバルから参照できるか確認する**

```bash
cat ~/.claude/skills/skill-routing/SKILL.md | head -10
```

Expected: frontmatter（`---` で囲まれた `name: skill-routing`）が見える。

- [ ] **Step 4: 成功基準をすべてチェックする**

```bash
# CLAUDE.md の Skill Routing セクションが 1 行か確認
grep -A 2 "## Skill Routing" ~/.claude/CLAUDE.md

# SKILL_ROUTING.md が削除されているか確認
ls /Users/kado/dotfiles/.claude/docs/

# skill-routing/ ディレクトリが存在するか確認
ls /Users/kado/dotfiles/.claude/skills/
```

Expected:
- `## Skill Routing` の直後に `` `skill-routing` スキルを使用して... `` の 1 行のみ
- `SKILL_ROUTING.md` が存在しない（`CODEX_OFFLOAD.md` のみ）
- `code-style  skill-routing  superpowers-doc-conventions` の 3 ディレクトリ
